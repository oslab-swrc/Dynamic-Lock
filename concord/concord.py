...
class LockGen(object):
    ...
    def _patch_kernel(self) -> None:
        lock_name = self.get_name(self.api_patch)
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument(
        '--lock', required=True,
        type=os.path.abspath,
        help='specify the patch file to expose Concord API for underlying lock'
    )
    ...
    # TODO: Now compile and run the VM:


if __name__ == '__main__':
    sys.exit(main(sys.argv[1:]))

#python3 concord.py -v --linux_src ~/rs3-policy-linux/src/linux --lock lockdir/shfllock.patch --policy policy/bravo --livepatch ../kpatch/patches/bravo-test.patch
